...
class ScrollNumber extends StatefulWidget {
...
class _NumberPickerState extends State<ScrollNumber> {
  ...
  void _scrollListener() {
    ...
    Future.delayed(const Duration(milliseconds: 100), () => _maybeCenterValue());
  }

  @override
  void didUpdateWidget(ScrollNumber oldWidget) {
  ...
  bool get needUpdate => widget.value != tempValue;
  ...
  Widget _itemBuilder(BuildContext context, int index) {
    ...
    final child = isExtra
        ...
        : Container(
            ...
            //   border: Border(bottom: BorderSide(color: appTheme.sliverColor)),
            // ),
            child: Text(_getDisplayedValue(value), style: itemStyle),
          );

    return Container(width: widget.itemWidth, height: widget.itemHeight, alignment: .center, child: child);
  }
  ...
  void _maybeCenterValue() {
    if (_scrollController.hasClients && !isScrolling && needUpdate) {
      tempValue = widget.value;
...
class _NumberPickerSelectedItemDecoration extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: IgnorePointer(
        child: Container(
          width: isVertical ? .infinity : itemExtent,
          height: isVertical ? itemExtent : .infinity,
